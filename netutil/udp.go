...
// ListenFromPacketConn creates a new listener from a net.PacketConn.
func ListenFromPacketConn(pConn *net.UDPConn) (net.Listener, error) {
	return (&ListenConfig{}).ListenFromPacketConn(pConn)
}

func (lc *ListenConfig) ListenFromPacketConn(pConn *net.UDPConn) (net.Listener, error) {
	if lc.Backlog == 0 {
...
func (l *listener) read() {
	...
	for {
		...
		if err != nil {
			l.errRead.Store(err)
			logger.Warnf("error reading from packet conn: %v", err)
			return
		}
...
func (l *listener) dispatchMsg(addr net.Addr, buf []byte) {
	...
	if ok {
		...
		if err != nil {
			logger.Errorf("error dispatching message addr=%v err=%v", addr, err)
		}
	}
}
